...
AgentDeck 1.0.0 — a physical-feeling command deck for AI coding agents.

- One-command hub: `npx agent-deck`, QR pairing with one-time tokens, LAN-only, no cloud.
- Claude Code managed sessions (Agent SDK) with deck approvals, diff previews, and a model/thinking dial; observed terminal sessions via `agent-deck connect claude` hooks, including deck-answered permission prompts.
