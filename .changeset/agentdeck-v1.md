...
- One-command hub: `npx agentdeck`, QR pairing with one-time tokens, LAN-only, no cloud.
- Claude Code managed sessions (Agent SDK) with deck approvals, diff previews, and a model/thinking dial; observed terminal sessions via `agentdeck connect claude` hooks, including deck-answered permission prompts.
