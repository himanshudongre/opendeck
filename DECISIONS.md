...
- **Switch acoustics are synthesized, not sampled** (2026-07-17): a key press is three layered WebAudio voices — click leaf (bandpass noise, clicky only), plate strike (bandpass noise), case resonance (decaying sine) — with ±5–8% per-strike detune so rolls never sound looped, a distinct quieter/brighter release strike, and a dry encoder tick for knob detents. Presets mirror switch families: `clicky`, `thocky`, `silent` (audible but dampened), `off`. No audio assets ship; a master compressor prevents clipping on fast rolls.
- **Press physics are asymmetric** like a real switch: 45 ms down-stroke, ~260 ms sprung release with slight overshoot (`cubic-bezier(0.18, 1.65, 0.4, 1)`), and the cap's extruded skirt (solid offset shadow) collapses on bottom-out. `prefers-reduced-motion` clamps the transition to 1 ms.
- **Bring-your-own-switch sounds** (2026-07-17): a `custom` preset plays the user's imported recording (IndexedDB via `switch-sounds.ts`, decoded through `OfflineAudioContext` so no user gesture is needed, ±3% per-press detune; optional separate release sample, else the press replays smaller and faster). Oversized (>2 MB) or undecodable files are rejected before storage; every degradation path (no IndexedDB, no WebAudio, bad file) leaves stock presets working.
- **The WebGL device face** (2026-07-17): micro mode renders as a real-time three.js scene — React 19 + @react-three/fiber v9 (v8 fails silently against three r185; the Clock deprecation warning was the tell), RoomEnvironment PMREM at 0.45 intensity, bloom + vignette via @react-three/postprocessing, spring integrators (not lerps) for key travel and stick return so caps overshoot and quiver. It ships as a lazy chunk (own 280 KB gz budget; boot keeps 300 KB) and falls back to the CSS face without WebGL2 or under reduced motion. Both faces share one brain: `micro-model.ts`. The 3D face mirrors controls in an `sr-only` DOM layer for screen readers and Playwright; `Micro3D.tsx` is excluded from jsdom coverage (no GPU) and covered by E2E instead. Camera framing is measured, not derived: project the plate corners and walk the camera until the worst corner hits ~0.95 NDC.
- **Embedded-browser verification quirk**: the Claude browser pane throttles requestAnimationFrame, so the 3D scene freezes on its first frames there; `__OPENDECK_SCENE__.advance(t)` drives frames manually when verifying. Real browsers and Playwright are unaffected.
- **Touch-first 3D usability pass** (2026-07-17, after real-device feedback): near top-down camera (74°) with viewport-adaptive row stretch (`stretchFor`) so the device fills phones edge-to-edge instead of floating in perspective letterbox; whole sockets are hit targets (handlers on the key group, not the cap face); the knob drags vertically like a DAW control (26 px per detent) instead of demanding a circular gesture; horizontal swipes on empty plate page through agents (native listeners on the wrapper — R3F's canvas pointer capture bypasses React's delegated events — and no time limit on the gesture, a 964 ms drag is still a swipe); the LCD leads with the pending question ("approve Edit? · title") so truncation never eats it; selection = brass ring + cap raised 0.08; GPU cost cut for phones (dpr ≤ 1.5, no MSAA — bloom smooths edges, `high-performance` context). `e2e/scripts/usetest.mjs` drives the real WebGL hit-testing (touch tap on ✓, plate swipe, knob drag) headlessly, no a11y shortcuts.
