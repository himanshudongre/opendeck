...
const capGeometry = new RoundedBoxGeometry(1.42, 0.5, 1.42, 4, 0.16);
const wideCapGeometry = new RoundedBoxGeometry(1.42, 0.42, 0.92, 4, 0.14);
const micGeometry = new RoundedBoxGeometry(4.2, 0.42, 0.92, 4, 0.14);
...
function PressableKey({
  ...
  useFrame((state, delta) => {
    ...
    const target = pressed && !disabled ? -KEY_TRAVEL : 0;
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, -0.18, 0]} receiveShadow>
        <boxGeometry args={[1.62, 0.14, 1.62]} />
      ...
      <group ref={group}>
        ...
        {outline && (
          <mesh position={[0, 0.27, 0]} rotation={[-Math.PI / 2, 0, 0]}>
            <ringGeometry args={[0.64, 0.675, 40]} />
...
function Knob({
  ...
  const { axis, valueIndex, steps, knobDeg, setIndex } = knob;
  ...
  const indexFromPoint = (point: THREE.Vector3): number => {
      ...
      <group ref={spin}>
        <mesh
          ...
          onPointerDown={(event) => {
            ...
            event.stopPropagation();
            setDragging(true);
            setIndex(indexFromPoint(event.point), false, feel.detent);
          }}
        >
...
function Lcd({
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = urgent ? '#ffb454' : '#c8cdd6';
    ctx.fillText(line.length > 42 ? `${line.slice(0, 41)}…` : line, 28, 66);
    ctx.font = '30px "IBM Plex Mono", monospace';
    ctx.fillStyle = '#6b7180';
    ctx.fillText(stats, 28, 122);
  ...
  return (
    <group position={position}>
      <mesh receiveShadow>
        <boxGeometry args={[6.9, 0.18, 1.35]} />
...
function DeviceScene({
  ...
  useFrame((state, delta) => {
    ...
    // Parallax fidget: the whole device leans a few degrees toward the finger.
  ...
  return (
    <group ref={rig}>
      ...
      <Lcd line={lcdLine} stats={lcdStats} urgent={pending !== undefined} position={[0, 0.1, -4]} />

      {/* Row A: knob · two agent keys · stick */}
      <Knob knob={knob} position={[-2.9, 0.15, -2]} feel={feel} />
      {agentKey(0, [-0.95, 0.15, -2])}
      {agentKey(1, [0.95, 0.15, -2])}
      <Stick targetId={attention?.id} position={[2.9, 0.15, -2]} feel={feel} />

      {/* Row B: four agent keys */}
      {agentKey(2, [-2.9, 0.15, -0.1])}
      {agentKey(3, [-0.95, 0.15, -0.1])}
      {agentKey(4, [0.95, 0.15, -0.1])}
      {agentKey(5, [2.9, 0.15, -0.1])}

      {/* Row C: command caps */}
      {commandKeys.map((binding, index) => (
        <PressableKey
          key={binding.id}
          position={[-2.9 + index * 1.94, 0.12, 1.7]}
...
function FrameCamera() {
  ...
  useEffect(() => {
    ...
    // walk the camera in or out until the worst corner sits at ~0.9 NDC.
    const corners = [
      new THREE.Vector3(4.5, 0, 5.9),
      new THREE.Vector3(-4.5, 0, 5.9),
      new THREE.Vector3(4.5, 0.4, -5.9),
      new THREE.Vector3(-4.5, 0.4, -5.9),
    ];
    const elevation = THREE.MathUtils.degToRad(56);
    let distance = 14;
    for (let step = 0; step < 24; step += 1) {
      camera.position.set(0, Math.sin(elevation) * distance, Math.cos(elevation) * distance + 0.6);
      ...
      if (worst >= 0.92 && worst <= 0.97) break;
      distance *= worst / 0.95;
    }
  }, [camera, size]);
  return null;
...
function Footer({ model, knobText }: { model: MicroModel; knobText: string }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-3 z-10 flex flex-col items-center gap-1.5">
      ...
      <p aria-hidden className="silkscreen">
        {knobText} · you can just ship things
      </p>
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        {/* The glow is the product: LEDs, LCD, and deskglow all bloom for
            real instead of faking it with sprites. */}
        <EffectComposer>
